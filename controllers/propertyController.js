...
// _email query param
exports.getPropertyByPropertyId = async (req, res, next) => {
    try {

        let getProperty = await prisma.Property.findUnique({
            where: {
                id: parseInt(req.query._propertyId)
            }
        })
        if (getProperty) {
            return new HTTPResponse(res, true, 200, null, null, { getProperty });
        }
